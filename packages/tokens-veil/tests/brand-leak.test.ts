...
describe('Veil brand-leak guard (PRD NFR-2.5)', () => {
  let css: string;
  let aliasWhitelist: string[];

  beforeAll(async () => {
    if (!existsSync(BRAND_CSS)) {
      throw new Error(`Veil dist/index.css missing. Run \`pnpm --filter @veil-ncss/tokens-veil build\` first.`);
    }
  ...
  it('only touches base aliases or --color-veil-* palette tokens', () => {
